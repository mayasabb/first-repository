...
def remove_extinct_colors(bubble_colors):
    grid_colors = BubblesGrid.get_grid_colors()
    stack_colors = Stack.get_stack_colors()
    existing_colors = []
    ...
    for color in stack_colors:
        if color not in existing_colors:
            existing_colors.append(color)
    index = 0
    while index < len(bubble_colors):
        if bubble_colors[index] not in existing_colors:
            bubble_colors.pop(index)
        else:
            index += 1


def checking_if_in_stack():
    for i in range(len(Stack.stack)):
        for j in range(len(consts.BUBBLE_START_COLORS)):
            if consts.BUBBLE_START_COLORS[j] in Stack.stack[i]["color"]:
                return True
